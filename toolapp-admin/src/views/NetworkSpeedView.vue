    ...
    <div class="data-page">
      ...
      <el-card shadow="hover">
        <el-table :data="rows" stripe v-loading="loading" style="width: 100%">
          ...
          <el-table-column prop="download_speed" label="下载速度(Mbps)" width="150" />
          <el-table-column prop="upload_speed" label="上传速度(Mbps)" width="150" />
          <el-table-column prop="ping" label="延迟(ms)" width="100" />
          <el-table-column prop="server_name" label="测试节点" min-width="150" />
...
async function loadData() {
  loading.value = true
  try {
    const result = await api.getTableData('network_speed_records', {
...
async function handleDelete(id) {
  try {
    await api.deleteRecord('network_speed_records', id)
