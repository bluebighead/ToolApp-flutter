...
class _ImageCompressPageState extends State<ImageCompressPage> {
  ...
  Future<void> _initSafeOutputDir() async {
    ...
    // 加载上次保存的 SAF 自定义路径记忆
    final saved = await SafPathMemory.load('image');
    if (mounted) {
      setState(() {
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.image, color: Colors.green, size: 28),
              ),
          ...
          children: [
            ...
            Row(
              children: [
                const SizedBox(
                    width: 80,
                    child: Text('质量', style: TextStyle(fontSize: 13))),
                Expanded(
                  child: Slider(
            ...
            Row(
              children: [
                const SizedBox(
                    width: 80,
                    child: Text('最大宽度', style: TextStyle(fontSize: 13))),
                Expanded(
                  child: Slider(
            ...
            Row(
              children: [
                const SizedBox(
                    width: 80,
                    child: Text('最大高度', style: TextStyle(fontSize: 13))),
                Expanded(
                  child: Slider(
                    ...
                    onChanged: _isCompressing
                        ? null
                        : (v) => setState(() => _maxHeight = v.round()),
                  ),
                ),
                SizedBox(
                  width: 50,
                  child: Text('${_maxHeight}px',
                      style: const TextStyle(fontSize: 13)),
                ),
              ],
            ),
  ...
  Widget _buildOutputPathSelector() {
    ...
    if (_safTreeUri != null && _safDirDisplayName != null) {
      ...
      final ext = _inputPath != null ? p.extension(_inputPath!).toLowerCase() : '.jpg';
      displayPath = _outputPath ?? _getDefaultOutputPath(ext);
    }
    return Card(
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(
              ...
              size: 20,
              color: _safTreeUri != null ? Colors.green : Colors.grey,
            ),
            const SizedBox(width: 8),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('输出路径', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  Text(
                    displayPath,
  ...
  String _getOutputFileName() {
    final ext = _inputPath != null ? p.extension(_inputPath!).toLowerCase() : '.jpg';
    final baseName =
        _inputPath != null ? p.basenameWithoutExtension(_inputPath!) : 'image';
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_durationMs > 0)
                  _buildResultRow('耗时', _formatDuration(_durationMs)),
              ],
            ),
          ),
        ),
  ...
  Future<void> _pickFile() async {
    try {
      ...
      AppLogger.e('ImageCompressPage', '选择文件失败：$e');
    }
  }
    ...
    try {
      ...
      if (treeUri != null) {
        ...
        setState(() {
          ...
          // 清除旧的文件路径式输出路径，改用 SAF
          _outputPath = null;
        });
        // 持久化保存 SAF 路径记忆
        SafPathMemory.save(type: 'image', treeUri: treeUri, dirDisplayName: dirName);
      }
    } on PlatformException catch (e) {
      AppLogger.e('ImageCompressPage', 'SAF 选择目录失败：${e.message}');
      if (mounted) {
      ...
      AppLogger.e('ImageCompressPage', '选择输出路径失败：$e');
    }
  }
  ...
  double? _getEstimatedCompressionRatio() {
    if (_advancedMode) {
      // 高级模式下根据质量估算：quality=30≈0.3, 60≈0.5, 85≈0.7, 100≈1.0
      return _quality * 0.006 + 0.1;
    } else {
      switch (_preset) {
  ...
  Future<void> _startCompress() async {
    ...
    final compressStartMs = DateTime.now().millisecondsSinceEpoch;
    try {
      // 压缩始终使用应用内部安全目录作为临时输出路径
      final ext = p.extension(_inputPath!).toLowerCase();
      final tempOutputPath = _getDefaultOutputPath(ext);
      ...
      final params = _advancedMode
          ...
          : ImageCompressParams.fromPreset(_preset);

      // 执行压缩（压缩到临时目录）
      final result = await CompressorService.compressImage(
      ...
      if (_safTreeUri != null) {
        try {
          ...
          final safUri = await _writeToSafDirectory(
            _safTreeUri!, fileName, result.outputPath, mimeType,
          );
          // 写入 SAF 成功后，更新输出路径显示
          finalOutputPath = '$_safDirDisplayName/$fileName';
          AppLogger.i('ImageCompressPage', '文件已写入 SAF 目录: $safUri');
        } catch (e) {
          // SAF 写入失败，使用临时目录的结果
          AppLogger.e('ImageCompressPage', '写入 SAF 目录失败，使用默认路径：$e');
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) {
        ...
        final finalResult = CompressResult(
          ...
          durationMs: endMs - compressStartMs,
        );
        setState(() {
          _result = finalResult;
          _durationMs = endMs - compressStartMs;
          _isCompressing = false;
          _progress = 1.0;
        });
        // 记录压缩历史
        _recordHistory(finalResult, params, endMs - compressStartMs);
      }
    } catch (e) {
      AppLogger.e('ImageCompressPage', '压缩失败：$e');
      if (mounted) {
        setState(() {
          _error = e.toString();
          _isCompressing = false;
  ...
  // 记录压缩历史
  Future<void> _recordHistory(CompressResult result, ImageCompressParams params, int durationMs) async {
    try {
      final presetLabel = _advancedMode ? '高级模式' : _preset.name;
      final paramsStr = _advancedMode
          ? '质量=$_quality%, 最大宽=$_maxWidth, 最大高=$_maxHeight'
          : '${_preset.name}模式';
      ...
      AppLogger.e('ImageCompressPage', '保存历史记录失败：$e');
    }
  }
}
