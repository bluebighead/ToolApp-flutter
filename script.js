/* ============================================================
   ...
   - 入场动画
   - 客户端二维码生成（指向当前官网地址）
   - 智能下载链接处理（GitHub Pages / 本地开发 / 服务器部署）
   QR 算法基于 Kazuhiko Arase 的 qrcode-generator（MIT License）
   的精简实现，Model 2, Version up to 10, Error Correction L
   ============================================================ */

/* -------- 下载服务器配置（发布到 GitHub Pages 时请修改此地址） --------
   如果你的服务器有公网访问地址，请将下面的 DOWNLOAD_SERVER
   设置为你的服务器地址，例如：
   - "https://your-server.example.com/"
   - "http://123.45.67.89:3000/"
   - "https://xxxxxx.cpolar.top/"  （内网穿透地址）
   留空时自动使用当前页面域名（适用于本地开发和服务器自带部署）
   ================================================================ */
const DOWNLOAD_SERVER = "";  // ← 在这里填写你的下载服务器地址，例如 "https://xxx.cpolar.top/"
const APK_FILE_NAME = "toolapp-1.34.0-release.apk";
...
(function setupDownloadLinks(){
    ...
    if (DOWNLOAD_SERVER) {
        // 配置了下载服务器 → 使用该地址
        downloadBaseUrl = DOWNLOAD_SERVER.endsWith('/') ? DOWNLOAD_SERVER : DOWNLOAD_SERVER + '/';
    } else if (isLocal) {
        // 本地开发模式 → 使用相对路径
        downloadBaseUrl = './downloads/';
    } else {
        // 其他环境（如 GitHub Pages）→ 使用当前域名作为下载服务器
        // 注意：如果 GitHub Pages 上没有 APK 文件，需要设置上面的 DOWNLOAD_SERVER
        downloadBaseUrl = window.location.origin + '/downloads/';
    }

    const downloadUrl = downloadBaseUrl + APK_FILE_NAME;

    // 更新所有 a[href*="toolapp-"] 的下载链接
    const links = document.querySelectorAll('a[href*="toolapp-"]');
    links.forEach(link => {
        link.href = downloadUrl;
    });

    // 记录供调试
    console.log('ToolApp 下载链接:', downloadUrl);
})();
